...
public class SyncMysqlReader extends DefaultMysqlReader {
    ...
    @Override public void work() {
        Pair<Long, Long> p = BasicDao.autoGetStartEndPoint(config.getSourceDataSource(), config.getSourceTableName(), config.getPrimaryKeyName());
        long thisLoopEndPoint = (config.getEndDelay() == null ? p.getRight() : p.getRight() - config.getEndDelay());
        while (currentPos < thisLoopEndPoint) {
            long theEnd = (thisLoopEndPoint - currentPos >= config.getReaderStepSize()) ? currentPos + config.getReaderStepSize() : thisLoopEndPoint;
